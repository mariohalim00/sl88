...
export function ShopAllPage() {
  ...
  const { searchTerm, setSearchTerm, filteredProducts } = useCatalogViewState(products);
      ...
      <div className="flex gap-8">
        <aside className="sticky top-28 hidden h-fit w-64 shrink-0 flex-col gap-8 self-start border-r border-[#d4c4ac]/40 pr-6 lg:flex">
          <section className="space-y-4">
            <h2 className="text-sm font-semibold tracking-widest text-[#1c1c15] uppercase">Categories</h2>
            <div className="space-y-3">
              {categories.map((category) => (
                <label key={category} className="flex items-center gap-3 text-sm text-[#504533]">
                  <input type="checkbox" className="h-4 w-4 rounded border-[#d4c4ac] accent-[#f4b400]" />
                  {category}
                </label>
              ))}
            </div>
          ...
          <section className="space-y-4 border-t border-[#d4c4ac]/40 pt-7">
            <h2 className="text-sm font-semibold tracking-widest text-[#1c1c15] uppercase">Material</h2>
        ...
        <main className="w-full space-y-6 pb-10 md:pb-16">
          <header className="space-y-4">
            <h1 className="font-heading text-3xl font-semibold text-[#1c1c15] md:text-4xl">Shop All</h1>
            <ShopAllFilterBar
          ...
          <CartSummary items={summary.lineItems} subtotal={summary.subtotal} onRemoveItem={remove} />
        </main>
      </div>
    </div>
  );
